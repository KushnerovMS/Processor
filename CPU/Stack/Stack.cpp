...
StackFunctionError StackPop (Stack * stack, void * value)
{
    ...
    if (stackOk)
    {
        if (stack -> Size != 0)
        {
            ...
            memset (stack -> Data + (stack -> Size) * stack -> ItemSize,
                    0,
                    stack -> ItemSize);
...
void* StackGet (Stack* stack, StackFunctionError* err)
{
    bool stackOk = true;
    stackOk = StackOk (stack);
    ...
    if (stackOk)
    {
        ...
        {
            if (err) *err = SUCCESS;
            return stack -> Data + (stack -> Size) * stack -> ItemSize;
        }
        else
        {
            if (err) *err = NO_ITEMS_ERROR;
            return nullptr;
        }
    }
    else
    {
        if (err) *err = ERROR_STACK;
        return nullptr;
    }
}
...
void StackDump__ (const Stack * stack, DumpMode mode, const bool isOk, const char * message)
{
...
#ifdef _DEBUG
    Logs.print ("stack <%s> [0x%08X] %s ",
                    (stack -> dtype)? stack -> dtype : "?", stack, (isOk)? "ok" : "ERROR");

    if (stack) Logs.print ("\"%s\" at %s() at %s (%d)\n",
                                stack -> dname, stack -> dfunction, stack -> dfile, stack -> dline);
#else
    Logs.print ("stack [0x%08X] %s\n", stack, (isOk)? "ok" : "ERROR");
#endif

    if (message) Logs.print ("%s\n", message);
    ...
    if (stack && mode != MIN_INFO)
    {
        ...
        {
            Logs.print ("\tfirst canary = %u, second canary = %u\n",
                                stack -> Canary1, stack -> Canary2);
            Logs.print ("\tdata hash = 0x%04X\n\n", stack -> DataHash);
        }

        Logs.print ("\tsize = %u\n", stack -> Size);
        Logs.print ("\tcapacity = %u\n", stack -> Capacity);
        Logs.print ("\titem size = %u\n", stack -> ItemSize);
        Logs.print ("\tdata[0x%08X]\n", stack -> Data);
        if (stack -> Data != nullptr && (mode == ALL_INFO ||  mode == SECURITY_INFO))
        {
            for (int i = 0; i < stack -> Capacity; i++)
            {
                ...
                if (stack -> dprintItem != nullptr)
                    (*stack -> dprintItem) (Logs.getFile(), (void*) (stack -> Data + i * stack -> ItemSize));
                else
#endif
                    defaultPrint(Logs.getFile(), stack -> Data + i * stack -> ItemSize, stack -> ItemSize);
                Logs.print ("\n");
            }
